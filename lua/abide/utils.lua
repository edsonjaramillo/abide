...
M.notify = function(message, level)
	...
	vim.notify(message, vim.log.levels[resolved_level:upper()], { title = "abide" })
end
...
M.inspect = function(value)
	if value == nil then
		vim.notify("nil", vim.log.levels.INFO, { title = "abide debug" })
		return
	end
	vim.notify(vim.inspect(value), vim.log.levels.INFO, { title = "abide debug" })
end
